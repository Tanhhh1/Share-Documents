...
export function DocumentFilter({ filters, onChange, onClear }: DocumentFilterProps) {
    ...
    return (
        <div className="page-filter">
            ...
            <Button className="custom-button page-filter-clear" onClick={onClear}>
                Xóa lọc
            </Button>
        </div>
    );
...
export function MyDocumentFilter({ filters, onChange, onClear }: DocumentFilterProps) {
    const [keyword, setKeyword] = useState(filters.keyword ?? "");
    const debouncedKeyword = useDebounce(keyword, 500);
